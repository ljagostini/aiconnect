...
namespace Percolore.IOConnect.Modbus
{
	public class USBClass
    {
        ...
        private int m_nInputReportLength = 1024;
        public bool isComunication = false;
        public byte[] arrReadByte = new byte[1024];
        ...
        private class Win32Wrapper
        {
            ...
            [DllImport("kernel32.dll", SetLastError = true)] public static extern IntPtr CreateFile([MarshalAs(UnmanagedType.LPStr)] string strName, uint nAccess, uint nShareMode, IntPtr lpSecurity, uint nCreationFlags, uint nAttributes, IntPtr lpTemplate);
        ...
        protected void ReadCompleted(IAsyncResult iResult)
        {
            ...
            try
            {
                for (int i = 0; i < this.arrReadByte.Length; i++)
                {
                    this.arrReadByte[i] = 0x00;
                }
                for (int i = 0; i < arrBuff.Length; i++)
                {
                    this.arrReadByte[i] = arrBuff[i];
                }

                this.isComunication = false;
            ...
            catch (IOException e) // if we got an IO exception, the device was removed
            {
				LogManager.LogError($"Dispositivo removido no módulo {this.GetType().Name}: ", e);
			}
			catch (Exception e)
			{
        ...
        public void Write(byte[] oOutRep)
        {
            try
            {
                this.isComunication = true;
                m_oFile.Write(oOutRep, 0, oOutRep.Length);
            }
            catch (Exception exx)
            {
				LogManager.LogError($"Erro no módulo {this.GetType().Name}: ", exx);
        ...
        public bool GetUSBDevice(UInt32 VID, UInt32 PID, ref List<DeviceProperties> ListOfDP, bool GetCOMPort, Nullable<UInt32> MI = null)
        {
    ...
    {
        public static bool connectUsb = true;
    }
}